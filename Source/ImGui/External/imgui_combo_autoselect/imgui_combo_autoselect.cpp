...
/*
 // Demo:
	static ImGuiAutoSelect::ComboAutoSelectData data = {{
					"",
	...
	if (ImGuiAutoSelect::ComboAutoSelect("my combofilter", data)) {
		// selection occurred
	}
...
bool ImGuiAutoSelect::ComboAutoSelectComplex(const char* label, char* input, int inputlen, int* current_item, bool(*items_getter)(void*, int, const char**), void* data, int items_count, ImGuiComboFlags flags)
{
	...
	if (arrowScroll && *current_item > -1) {
		...
		const int new_len = static_cast<int>(strlen(sActiveidxValue2));
		state->TextA.resize(new_len + 1);  // +1 for null terminator
		memcpy(state->TextA.Data, sActiveidxValue2, new_len + 1);
		state->Stb->cursor = new_len;
		state->Stb->select_end = new_len;
		state->Stb->select_start = new_len;
		state->CursorClamp();
...
bool ImGuiAutoSelect::ComboAutoSelect(const char* label, ImGuiAutoSelect::ComboAutoSelectData & data, ImGuiComboFlags flags)
{
